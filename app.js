...
/* =======================
   ...
   ======================= */
/* Waze / Google Maps: "Ruta desde aquí" */
...
function mapsLinkFromLatLon(lat, lon) {
...
function parseLatLonFromGMaps(url) {
  ...
  try {
    ...
    // 4) Primera pareja lat,lon razonable
    {
      const m = u.match(/(-?\d+(?:\.\d+)?),\s*(-?\d+(?:\.\d+)?)/);
      if (m) {
...
/* =======================
   NUEVO: Botones de agregar lugares
   - "+ Agregar lugar": formulario normal
   - "📍 Agregar desde aquí": usa geolocalización y auto-rellena lat/lon
   ======================= */
(function injectAddPlaceUI() {
  // Estilos mínimos del modal (inyectados para no tocar tu CSS)
  const css = `
  ...
  .addplace-row input,.addplace-row select{padding:10px 12px;border:1px solid #d1d5db;border-radius:10px;font-size:14px}
  ...
  addHereBtn.className = "btn";
  addHereBtn.textContent = "📍 Agregar desde aquí";
  addHereBtn.title = "Usa tu ubicación actual para auto-rellenar coordenadas";

  toolbar.appendChild(addBtn);
  toolbar.appendChild(addHereBtn);
    ...
    <div class="addplace-modal" role="dialog" aria-modal="true" aria-labelledby="ap-title">
      ...
      <div class="addplace-body">
        ...
        <p style="font-size:12px;color:#6b7280;margin:4px 0 0">
          Con “📍 Agregar desde aquí” autocompletaremos <strong>lat/lon</strong> y un <em>link</em> de Maps.
          Igual debes completar <strong>Tipo</strong>, <strong>Nombre</strong> y <strong>Distrito</strong>.
        </p>
      </div>
  ...
  function openModal() {
    // Si el usuario tenía tipos seleccionados, propone el primero
    const preferredType = Array.from(selectedTypes)[0] || "Parque";
    $type().value = preferredType;
    $name().value = "";
    $district().value = "";
    $link().value = "";
    $lat().value = "";
  ...
  function openModalWithCoords(lat, lon) {
    ...
    // Prellenar link para que funcione aunque editen
  ...
  addHereBtn.addEventListener("click", () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        const { latitude: lat, longitude: lon } = pos.coords;
        // también actualizamos userPos para mejorar distancia/orden
        userPos = { lat, lon };
        saveUserPos();
        if ($sort.value !== "distance") {
          $sort.value = "distance";
        }
        // abrir modal precargado con coords
